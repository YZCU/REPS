% -------------------------------------------------------------------------------------------------
function [bboxes,fps] = otb_tracker(param)
%TRACKER
...
% -------------------------------------------------------------------------------------------------
    ...
    addpath('/home/ubuntu/visual_tracker_benchmark/trackers/SiamFC/tracking');
    p.numScale = 3;%3  
    p.scaleStep = 1.0375;
    ...
    p.visualization=param.visualization;
    p.gpus=param.gpus;
% -------------------------------------------------------------------------------------------------
...
%  
      imgFiles=param.s_frames;
      targetPosition=param.init_pos;
      targetSize=param.wsize;
    ...
    fps = (nImgs-startFrame+1)/time;
    bboxes = bboxes(startFrame : i, :);
    
    %% changed by linlin
    %dlmwrite([p.video,'.txt'], int16(bboxes)); 
end
