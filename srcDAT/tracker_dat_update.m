function [state, location] = tracker_dat_update(state, I, varargin)
  ...
  I1 = uint8(gather(I(:,:,:))); %自己加的
  
  img_preprocessed = state.imgFunc(imresize(I1, state.scale_factor));
  ...
  target_pos = [target_pos(1,2) target_pos(1,1)]; % 自己变换了xy的坐标
  target_sz = prev_sz .* state.scale_factor;
  target_sz = [target_sz(1,2) target_sz(1,1)];% 自己变换了wh
  ...
  % Localization visualization 可视化
  if cfg.show_figures
    figure(2), clf
  ...
  location = pos2rect(state.target_pos_history(end,:), state.target_sz_history(end,:), [size(I1,2) size(I1,1)]);
